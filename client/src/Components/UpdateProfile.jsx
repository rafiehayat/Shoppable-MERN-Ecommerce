...
export default function UpdateProfile() {
    ...
    async  function postData(e){
        ...
        var response = await fetch(`${apiLink}/api/user/`+localStorage.getItem("userid"),{
            method:"put",
            headers: {
    ...
    <>
      <div className="container-fluid my-3 w-100">
        <div className="w-75 m-auto">
            <h5 className='text-center btn-color p-2 text-light'><span className='text-warning fs-4'>Update </span> Profile </h5>
                ...
                <div className="mb-3">
                    <div className="btn-group w-100">
                       <button className='btn btn-success' onClick={()=>window.history.back()} >Back</button>
                        <button type='submit' className='btn btn-color text-light'>Update</button>
                    </div>
                </div>
            </form>
        </div>
