...
.is().not().oneOf(['Passw0rd', 'Password123', 'User123','User@123']); // Blacklist these values
...
async function createRecord(req,res){
  if(req.body.password && schema.validate(req.body.password)){
        ...
        bcrypt.hash(req.body.password,12,async(error,hash)=>{
            ...
            mailOptions = {
                ...
                to: data.email,
                subject:"Account Is Created : Team Kifayati",
                text:`
                        Hello ${data.name}
                        Your Kifayti Account is Successfully Created
                        Team : Kifayti
                       `
            }
            ...
            res.send({status:200,result:"Done",data:data})
                ...
                res.send({status:400,result:"failed",message:"User Name Already Taken"})
                else if(error.keyValue && error.keyValue.email)
                res.send({status:400,result:"failed",message:"Email Address Already Taken"})
  ...
  res.send({status:400,result:"Failed",message:"Password Must Contains Following Items 1.Minimum length 8 2.Maximum length 30 3.Must have 1 uppercase letters  4.Must have 1 lowercase letters 5.Must have at least 1 digits 6.Should not have spaces "})
}
...
async function updateRecord(req,res){
    try{
        ...
        if(error.keyValue)
        res.send({status:400,result:"failed",message:"User Name Already Exist"})
        else
        res.send({status:500,result:"failed",message:"Internal Server Error"})
    }
}
...
async function login(req,res){
    try {
      ...
      if(data){
            if(await bcrypt.compare(req.body.password,data.password)){
                  ...
                  jwt.sign({data},key,{expiresIn:"365d"},(error,token) =>{       //"15 days", 1296000
                      if(error)
                      res.send({status:500,result:"failed",message:"Internal Server Error"})
                      else
            ...
            res.send({status:401,result:"failed",message:"Invalid Username Or Password"}) 
     }
      else
        res.send({status:401,result:"failed",message:"Invalid Username Or Password"})
    } catch (error) {
 ...
 async function forgetPassword1(req,res){
      try{
       ...
       if(data){
        ...
        await data.save()
         mailOptions = {
            from: process.env.MAIL_SENDER,
            to: data.email,
            subject:"OTP For Password Reset : Team Kifayati",
            text:`
                    Hello ${data.name}
       ...
       res.send({ status: 401, result: "failed", message: "User Not Found"})
      }catch(error){
        res.send({ status: 500, result: "failed", message: "Internal Server Error"})
      }
 }
 async function forgetPassword2(req,res){
    try{
     ...
     if(data){
      ...
      res.send({ status: 401, result: "failed", message: "Invalid OTP"})   
      }
     else
     res.status(401).send({ status: 401, result: "failed", message: "Anauthorized Activity"})
    }catch(error){
      res.send({ status: 500, result: "failed", message: "Internal Server Error"})
    }
}
