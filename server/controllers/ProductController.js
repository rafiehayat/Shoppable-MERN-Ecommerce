...
async function createRecord(req, res) {
  try {
    ...
    if (error.errors.name)
      res.send({ status: 500, result: "failed", message: error.errors.name.message });
    else if (error.errors.maincategory)
      res.send({ status: 500, result: "failed", message: error.errors.maincategory.message });
    else if (error.errors.subcategory)
      res.send({status: 500,result: "failed", message: error.errors.subcategory.message });
    else if (error.errors.brand)
      res.send({ status: 500, result: "failed", message: error.errors.brand.message });
    else if (error.errors.color)
      res.send({ status: 500,result: "failed", message: error.errors.color.message });
    else if (error.errors.size)
      res.send({status: 500,result: "failed", message: error.errors.size.message});
    else if (error.errors.baseprice)
      res.send({ status: 500, result: "failed", message: error.errors.baseprice.message});
    else if (error.errors.discount)
      res.send({ status: 500, result: "failed",message: error.errors.discount.message });
    else if (error.errors.finalprice)
      res.send({
        status: 500, result: "failed",message: error.errors.finalprice.message });
    else
      res.send({status: 500,result: "failed", message: "Internal Server Error" });
  }
}
...
async function updateRecord(req, res) {
  try {
    ...
    if (error.keyValue)
      res.send({status: 400,result: "failed",message: "Product Name Already Exist"});
    else
      res.send({status: 500, result: "failed",message: "Internal Server Error" });
  }
}
